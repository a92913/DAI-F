...
const getLikes = () => {
    ...
    fetch(url + "ActivityGetLikeEE?cod_ee=" + cod)
        ...
        .then((output) => {
            ...
            output.MSG.forEach(({ cod_act, name, age_low, age_upper, date, start_time, duration, soft_skills, description, photo, category, namechild, cod_child }) => {
                ...
                if (photo == null || photo == "null" || photo == "" || typeof photo === typeof undefined) {
                ...
                const content = getLikesChild(cod_act, name, age_low, age_upper, date, start_time1, duration1, soft_skills, description, src, category, namechild, cod_child);
...
const getLikesChild = (cod_act, name, age_low, age_upper, date, start_time, duration, soft_skills, description, photo, category, namechild, cod_child) => {
    ...
    <div id="show-card">
        ...
        <ul class="dropdown-content ${category}" id="${cod_act + cod_child +  "a"}">
            ...
            <div id="div-heart">
                <button class="inscribe" value="${cod_act}" onclick="inscribeKid(this.value)">
                    Inscrever
                </button>
...
function inscribeKid(valor) {
    localStorage.setItem("cod_act", valor);
    ...
    fetch(url + "ChildrenInscription?cod_ee=" + id + "&cod_act=" + cod_act)
        ...
        .then((output) => {
            ...
            if (output.MSG.length == 0) {
                Swal.fire({
                    text: "Não tem educandos para inscrever!",
                    icon: "error",
                    confirmButtonText: 'Ok',
                });
            } else {
                window.location.replace("childrenInscription.html")
            }
        });
};

localStorage.setItem("page", "like.html");
